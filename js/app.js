...
const GRADES=[
 {id:"g1",label:"１ねんせい",stages:[
   ...
   {id:"g1_kr",name:"かんじ よみ",emoji:"📖",subject:"kokugo",time:15,gen:()=>kanjiRead(KANJI1)},
   {id:"g1_kw",name:"かんじ かき",emoji:"✏️",subject:"kokugo",time:15,gen:()=>kanjiWrite(KANJI1)},
 ...
 {id:"g2",label:"２ねんせい",stages:[
   {id:"g2_kr",name:"かん字 よみ",emoji:"📗",subject:"kokugo",time:15,gen:()=>kanjiRead(KANJI2)},
   {id:"g2_kw",name:"かん字 かき",emoji:"🖊️",subject:"kokugo",time:15,gen:()=>kanjiWrite(KANJI2)},
...
function load(){
  const def={v:2,grade:"g1",stars:{},best:{},stats:{},hist:[],wrong:[],streak:{last:"",n:0},mute:false,
    tickets:0,ticketProg:0,doneToday:{d:"",ids:[]},wrongProg:0};
...
/* ── ゲームチケット ── */
function markDoneToday(id){const t=today();
  if(!DB.doneToday||DB.doneToday.d!==t)DB.doneToday={d:t,ids:[]};
  const first=!DB.doneToday.ids.includes(id);
  if(first)DB.doneToday.ids.push(id);
  return first;}
function maxActiveGradeIdx(){let idx=GRADES.findIndex(g=>g.id===DB.grade);if(idx<0)idx=0;
  GRADES.forEach((g,i)=>{if(g.stages.some(s=>(DB.stars[s.id]||0)>0))idx=Math.max(idx,i);});
  return idx;}
function grantTickets(gain){DB.ticketProg=(DB.ticketProg||0)+gain;let earned=0;
  while(DB.ticketProg>=1&&(DB.tickets||0)<MAX_TICKETS){DB.ticketProg-=1;DB.tickets=(DB.tickets||0)+1;earned++;}
  if((DB.tickets||0)>=MAX_TICKETS)DB.ticketProg=Math.min(DB.ticketProg,1);
  return earned;}
...
function startQuiz(stage){
  ...
  while(qs.length<N&&guard++<200){
    const q=stage.gen();if(seen.has(q.q+(q.big||"")))continue;seen.add(q.q+(q.big||""));qs.push(q);}
...
function renderQ(){
  ...
  <div class="qcard">
    <div class="qtext">${esc(q.q).replace(/\n/g,"<br>")}</div>
    ${q.big?`<div class="qbig">${esc(q.big)}</div>`:""}
  </div>
  <div class="choices">${q.choices.map((c,ci)=>`
    <button class="choice focusable" data-act="ans" data-i="${ci}">${esc(c)}</button>`).join("")}
  </div>`;
...
function finishQuiz(){
  ...
  bumpStreak();
  let ticketEarned=0,ticketGain=0;
  if(!stage.review){
    const prevBest=DB.best[stage.id]||0;
    const isRecord=score>prevBest;                              // 記録更新なし(周回)は0
    const firstToday=markDoneToday(stage.id);                  // 今日はじめてなら true
    const stageIdx=GRADES.findIndex(g=>g.stages.some(s=>s.id===stage.id));
    const gradeMul=stageIdx>=maxActiveGradeIdx()?1.0:0.3;      // 下の学年は0.3
    ...
    // チケット = 基本1 ×(新記録1.0/周回0) ×(今日未着手1.0/既習0.3) ×(現学年1.0/下学年0.3) ×(acc>=70で1.0)
    ticketGain=1*(isRecord?1.0:0)*(firstToday?1.0:0.3)*gradeMul*(acc>=70?1.0:0);
    if(DB.wrong.length>=10)ticketGain=0;                       // まちがい10以上は付与停止(ノート優先)
    ticketEarned=grantTickets(ticketGain);
  ...
  renderResult({stage,acc,cor,total:qs.length,score,stars,time:Math.round((Date.now()-quiz.t0)/1000),
    ticketEarned,ticketGain,tickets:DB.tickets||0,ticketProg:DB.ticketProg||0,wrongProg:DB.wrongProg||0});
}
function renderResult(r){
  ...
  if(!r.stage.review){
    if(r.ticketEarned>0){
      tk=`<div class="tk-box">🎟️ ゲームチケットを ${r.ticketEarned}まい ゲット！
        <small>もっている チケット：${r.tickets}まい</small></div>`;
    }else{
      const pct=Math.round((r.ticketProg||0)*100);
      const why=r.acc<70?"せいかい 70% いじょうで もらえるよ"
        :DB.wrong.length>=10?"まちがいノートが 10もん いじょう。さきに ふくしゅうしよう！"
        :"新記ろくを だすと チケットが たまるよ";
      tk=`<div class="tk-box">🎟️ つぎの チケットまで ${pct}%<small>${why}</small>
        <div class="tk-bar"><i style="width:${pct}%"></i></div></div>`;
    }
  }else{
    if(r.ticketEarned>0){
      tk=`<div class="tk-box">🎟️ まちがいノートを がんばったので ゲームチケットを ${r.ticketEarned}まい ゲット！
        <small>もっている チケット：${r.tickets}まい</small></div>`;
    }else{
      const left=WRONG_NOTE_GOAL-(r.wrongProg||0);
      const pct=Math.round((r.wrongProg||0)/WRONG_NOTE_GOAL*100);
      tk=`<div class="tk-box">🎟️ まちがいノートを あと ${left}問 正解で チケット1まい
        <small>まちがえた 問題を ${WRONG_NOTE_GOAL}問 正解すると ゲームチケットが もらえるよ</small>
        <div class="tk-bar"><i style="width:${pct}%"></i></div></div>`;
    }
  }
  app.innerHTML=`
